...
public class PopupDialog extends Window {
	...
	public PopupDialog(Skin skin, MyListItem[] gridNames) {
	...
	private void initialize() {
		...
		java.util.List<MyListItem> items = new ArrayList<MyListItem>();
		for (MyListItem it : gridNames) {
			if (it == null) continue;
			items.add(it);
		}

		
		list = new List<MyListItem>(skin);
		list.setItems(items.toArray(new MyListItem[items.size()]));
		...
		deleteBtn.addListener(new ChangeListener() {
					public void changed (ChangeEvent event, Actor actor) {
						for (int i = 0;i<gridNames.length;i++) {
							if (gridNames[i] == selectedItem) gridNames[i] = null;
						}
					}
				});
		internalTable.row();
