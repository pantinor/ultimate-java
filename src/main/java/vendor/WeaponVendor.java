	...
	public boolean nextDialog() {
		...
		case WAIT_BUY_INPUT:
			...
			for (Item i : vendor.getInventoryItems()) {
				list += i.getChoice() + "-" + i.getName()+" for " + i.getPrice() + "gp.\n";
			}
            list += "Your Interest?";
		...
		case BUY_ITEM:
			party.getSaveGame().weapons[currentSelectedItem.getWeaponType().ordinal()] = currentCount;
		...
		case WAIT_SELL_INPUT:
			...
			for (Item i : vendor.getVendorClass().getItemCatalog()) {
				if (party.getSaveGame().weapons[i.getWeaponType().ordinal()] > 0) {
					sellList += i.getChoice() + "-" + i.getName()+" for " + i.getPrice()/2 + "gp.\n";
				} else {
					continue;
				}
			}
		...
		case WAIT_SELL_DONT_HAVE_THAT_MANY:
			displayToScreen("You don't have that many swine!");
			state = ConvState.ASK_BUY_SELL;
			break;
