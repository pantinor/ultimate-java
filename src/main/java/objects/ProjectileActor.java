...
public class ProjectileActor extends Actor {
	...
	public ProjectileActor(BaseScreen screen, Color color, int x, int y, AttackResult res) {
		...
		if (color == Color.RED) texture = redBall;
		if (color == Color.BLUE) texture = blueBall;
		if (color == Color.WHITE) texture = whiteBall;
		if (color == Color.GREEN) texture = greenBall;
		if (color == Color.TEAL) texture = tealBall;
		if (color == Color.PURPLE) texture = purpleBall;
